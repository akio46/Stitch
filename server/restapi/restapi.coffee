...
Api.addRoute 'updateCollections', authRequired: false,
	get: ->
		subscriptions = RocketChat.models.Subscriptions.find().fetch();

		for sub in subscriptions
			user = RocketChat.models.Users.findOneById(sub.u._id)
			room = RocketChat.models.Rooms.findOneById(sub.rid)
			...
			if room.t is 'd'
				...
				RocketChat.models.Rooms.update query, update

				for username in room.usernames
					if username isnt user.username
						toUser = RocketChat.models.Users.findOneByUsernameAndOrg(username, user.organizationId)
						...
						RocketChat.models.Subscriptions.update query, update

		organizations = RocketChat.models.Organizations.find().fetch();
		...
		for org in organizations
			if org.domain
				...
				RocketChat.models.Organizations.update query, update
...
Api.addRoute 'publicRooms', authRequired: true,
	get: ->
		...
		rooms = RocketChat.models.Rooms.findByType('c', orgId, { sort: { msgs:-1 } }).fetch()
