...
Template.room.helpers
			...
			rules: [
				{
					...
					matchAll: true
					filter: { uid: { $ne: Meteor.userId() }, organizationId: Meteor.user().organizationId }
					sort: 'name'
				}
			]
		}
	...
	canEditName: ->
		...
		if roomData.t in ['c', 'p']
			roomName = RoomManager.getRoomName(this._id)

			return (roomName isnt 'general') and RocketChat.authz.hasAtLeastOnePermission('edit-room', this._id)
		else
			return ''
...
Template.room.events
	...
	'click .delete-room': (e) ->
		...
		self = this
		roomName = RoomManager.getRoomName(self._id)

		swal {
			title: 'Delete #' + roomName
			text: 'Deleting self group will delete all of the messages in ' + 'it, for everyone in your team, forever.' + ' To confirm, enter <strong>' + roomName + '</strong> below.'
			html: true
			type: 'input'
			showCancelButton: true
			closeOnConfirm: false
			confirmButtonText: 'Delete ' + roomName
			confirmButtonColor: '#ec6c62'
		}, (inputValue) ->
			if inputValue == roomName
				Meteor.call 'deleteRoom', self._id, (error, result) ->
					if error
						swal
						...
						swal
							...
							text: 'The <strong>#' + roomName + '</strong> ' + 'channel is gone forever!'
							type: 'success'
							html: true
						Session.set 'channelsList', result.channels
						RoomManager.close self._id
						FlowRouter.go 'home'
					return
			else
				swal
	...
	updateUnreadCount = _.throttle ->
		...
		if firstMessageOnScreen?.id?
			...
			if firstMessage?
				...
				template.unreadCount.set ChatMessage.find({'u._id': Meteor.userId(), rid: template.data._id, ts: {$lt: firstMessage.ts, $gt: subscription.ls}}).count()
			else
